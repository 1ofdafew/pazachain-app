...
export function WalletHeader({ isConnected, onConnect, onDisconnect, address, pusdBalance = "0.00" }: WalletHeaderProps) {
  const [copied, setCopied] = useState(false)
  
  const displayAddress = address ? `${address.slice(0, 6)}...${address.slice(-4)}` : ""
  const formattedBalance = Number.parseFloat(pusdBalance).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })
    ...
    <header className="sticky top-0 z-50 w-full border-b border-border bg-background/80 backdrop-blur-xl">
      <div className="flex h-14 items-center justify-between px-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-base font-semibold text-foreground tracking-tight">PAZA</span>
        </div>
