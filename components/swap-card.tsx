...
import { ArrowDown, ChevronDown, Scan, X, Copy, Check, ExternalLink } from "lucide-react"
...
const QRCode = dynamic(
  ...
  { ssr: false, loading: () => <div className="w-[160px] h-[160px] bg-muted animate-pulse rounded-lg" /> }
)
  ...
  return (
    <div className="mt-4 p-4 bg-primary/10 border border-primary/20 rounded-xl space-y-3">
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm font-medium text-foreground">Transaction Submitted</span>
      </div>
...
export function SwapCard({ activeTab, onTabChange, isConnected, onConnect, pusdBalance = "0.00" }: SwapCardProps) {
  ...
  const [buyTxResult, setBuyTxResult] = useState<{ message: string; txHash: string } | null>(null)
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 bg-secondary rounded-xl p-3 ring-1 ring-primary/20">
                  ...
                  <div className="flex items-center gap-2 bg-muted rounded-lg px-3 py-2 shrink-0">
                    ...
                    <span className="text-sm font-medium text-foreground">PUSD</span>
...
function SendTab({ isConnected, onConnect }: { isConnected: boolean; onConnect: () => void }) {
  const [amount, setAmount] = useState("")
  const [recipient, setRecipient] = useState("")
  const [selectedToken, setSelectedToken] = useState<TokenType>("PAZA")
  const [isTokenMenuOpen, setIsTokenMenuOpen] = useState(false)
  const [isScanning, setIsScanning] = useState(false)
  const [sendTxResult, setSendTxResult] = useState<{ message: string; txHash: string } | null>(null)
    ...
    try {
      ...
      await scanner.start(
        ...
        (decodedText) => {
          ...
          const cleanAddress = decodedText.replace(/^ethereum:/i, "").split("@")[0]
            ...
            <div className="flex items-center gap-2 bg-secondary rounded-xl p-3">
              ...
              <div className="relative">
                ...
                >
                  ...
                  <span className="text-sm font-medium text-foreground">{selectedToken}</span>
                  <ChevronDown className={`w-4 h-4 text-muted-foreground transition-transform ${isTokenMenuOpen ? "rotate-180" : ""}`} />
                </button>
...
function ReceiveTab({ isConnected, onConnect }: { isConnected: boolean; onConnect: () => void }) {
